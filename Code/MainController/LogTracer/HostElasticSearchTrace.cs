...
namespace MainController
{
    ...
    public class HostElasticSearchTrace : LoggerInterface
    {
        // Configuración // Configuration
        private static void ConfigureLogging()
        {
            var environment = Environment.GetEnvironmentVariable("ASPNETCORE_ENVIRONMENT");
            // Carga la configuración de la appsettings.json y buildea
            // Load the configuration from appsettings.json and build
            var configuration = new ConfigurationBuilder()
                ...
                .AddJsonFile(
                    ...
                    optional: true)
                .Build();
            // Crea el Logger, con especial énfasis en WriteTo.Elasticsearch
            // Creates the Logger, with special emphasis on WriteTo.Elasticsearch
        ...
        private static ElasticsearchSinkOptions ConfigureElasticSink(IConfigurationRoot configuration, string environment)
        {
            return new ElasticsearchSinkOptions(new Uri(configuration["ElasticConfiguration:Uri"]))
            {
                ...
                IndexFormat = $"Bitboss-MainController-Host-{DateTime.UtcNow:yyyy-MM}" // Bitboss-MainController-*
            };
        }
