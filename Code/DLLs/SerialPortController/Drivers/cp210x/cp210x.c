...
#define DRIVER_DESC "CP210x RS232 serial adaptor driver"

/*
 * Function Prototypes
 */

static int cp210x_open(struct tty_struct *tty, struct usb_serial_port *); // Open port
static void cp210x_close(struct usb_serial_port *); // Close port
static int cp210x_ioctl(struct tty_struct *tty, 
	unsigned int cmd, unsigned long arg); // Ioctl command to port
static void cp210x_get_termios(struct tty_struct *, struct usb_serial_port *); // Get termios struct
static void cp210x_get_termios_port(struct usb_serial_port *port,
	unsigned int *cflagp, unsigned int *baudp); // Get termios port
static void cp210x_change_speed(struct tty_struct *, struct usb_serial_port *,
							struct ktermios *); // Change speed to port
static void cp210x_set_termios(struct tty_struct *, struct usb_serial_port *,
							struct ktermios*); // Set Termios to port
static bool cp210x_tx_empty(struct usb_serial_port *port); // Clear Tx buffer
static int cp210x_tiocmget(struct tty_struct *); // TIOCMGet
static int cp210x_tiocmset(struct tty_struct *, unsigned int, unsigned int); // TIOCMSet
static int cp210x_tiocmset_port(struct usb_serial_port *port, 
		unsigned int, unsigned int); // TioCMSet Port
static void cp210x_process_read_urb(struct urb *urb); /* Reads an URB (USB Request block). An URB is a message containing all the relevant information for execute any usb transaction.*/
static void cp210x_enable_event_mode(struct usb_serial_port *port); /* Enable event mode for that serial port */
static void cp210x_disable_event_mode(struct usb_serial_port *port); /* Disable event mode for that serial port */
...
/* The state type cp210x_event_state includes the following states */
...
/* We define an structure named cp210x_port_private whose elements are: */
...
/* We define the cp210x_device as a usb serial driver. That driver is a structure defined in the kernel, that contains an element called process_read_urb. That element is a method that reads an URB (USB Request block). An URB is a message containing all the relevant information for execute any usb transaction.*/
...
{
	struct usb_serial *serial = port->serial; // Let serial de serial port name
	struct cp210x_port_private *port_priv = usb_get_serial_port_data(port); // Let port_priv the serial port private
	void *dmabuf; 
	int result;

	dmabuf = kmalloc(bufsize, GFP_KERNEL); // Alloc a dmabuf with bufsize elements
	if (!dmabuf) {
		/*
	...
	result = usb_control_msg(serial->dev, usb_rcvctrlpipe(serial->dev, 0),
			...
			USB_CTRL_SET_TIMEOUT); /* Sends a control msg of REQTYPE_INTERFACE_TO_HOST type */ 
	if (result == bufsize) {
		memcpy(buf, dmabuf, bufsize);/* Success */
		result = 0;
	} else {
...
static int cp210x_open(struct tty_struct *tty, struct usb_serial_port *port)
{
	...
	if (result) {
		...
		return result;
      	...
      	result = cp210x_write_u16_reg(port, CP210X_EMBED_EVENTS, CP210X_ESCCHAR);
	...
	if (tty) {
	   ...
	   /* I_INPCK esta en include/linux/tty.h.
	    * Es una macro o funcion que dice si esta habilitado el chequeo de paridad en el input */
           if (I_INPCK(tty))
...
/* The method has the following behahior:
 * Given an urb, from this element the method obtains the context.
 * That context provides the serial port, P. The method obtains the transfer_buffer , namely ch where the message is hosted. If the event mode of P is enabled, it start to analyze the message ch, byte to byte. When a byte is analyzed, the method will process the state machine transitions */
...
/* To enable the event mode, it obtains the serial port (for example /dev/ttyUSB0) with the structure cp210x_port_private. If the serial port is already in the event mode, it does nothing. If not, it sets the event_state in ES_DATA and the event_mode in true. Then it sends a command CP210X_EMBED_EVENTS with the scape char CP210X_ESCCHAr to the chip */
...
static void cp210x_set_termios(struct tty_struct *tty,
		struct usb_serial_port *port, struct ktermios *old_termios)
{
	/* Al inicializar el termios, despues de ciertas operaciones se consulta el I_INPCK, y si esta habilitado el  chequeo de paridad, se setea el modo eventos del cp210x, caso contrario se deshabilita */
	struct device *dev = &port->dev;
	...
	if ((cflag & CRTSCTS) != (old_cflag & CRTSCTS)) {
        ...
        /* As we said before, this is a macro or function that returns true if the parity checking is set, or false otherwise*/
	if (I_INPCK(tty))
		cp210x_enable_event_mode(port);
	else
